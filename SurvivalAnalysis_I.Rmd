...
- Some <u>basic </u> R commands
  - loading pacakges, `R` functions, plotting etc.
...
>`r text_spec("Hazard probability:", background = "#D05A6E", color = "white", bold = T)` Probability that a patient will have the even of interest at time *t* (denoted by *h(t)*), given that the individual has survived up to time *t*.
...
```{r, eval=FALSE, echo=F, message=FALSE, warning=FALSE, comment=""}
# writexl::write_xlsx(ovarian, "/Users/jayawardanamadawa/Desktop/1.PETERMAC/2.WORK/LABS/1.General/1.Workshops/1.Survival Analysis/Data/ovarian.xlsx")
```
...
### Loading data
```{r, echo=T, message=FALSE, warning=FALSE, comment=""}
JCO_orig <- readxl::read_excel("/Users/jayawardanamadawa/Documents/data_kathryn.xlsx", sheet = 1)
...
JCO_data$`Gene mutation found in` <- mapvalues(JCO_data$`Gene mutation found in`,
         from = c('BRCA1', 'BRCA2'), to = rep('Combined_positive', 2))
...
JCO_final$Status <- mapvalues(JCO_final$Status, 
        from = c('D', 'P'), to = c(1, 0))
